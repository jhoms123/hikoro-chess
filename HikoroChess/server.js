...
const { getInitialBoard, getValidMovesForPiece, isPositionValid, pieceNotation } = require('./gamelogic'); // Assuming pieceNotation is exported

// --- NEW HELPER FUNCTIONS ---
function coordsToNotation(x, y) {
    const file = String.fromCharCode('a'.charCodeAt(0) + x);
    const rank = BOARD_HEIGHT - y; // y=0 is rank 16, y=15 is rank 1
    return file + rank;
}

function generateNotation(piece, from, to, isCapture) {
    const pieceChar = pieceNotation[piece.type] || piece.type.charAt(0).toUpperCase();
    const fromSquare = coordsToNotation(from.x, from.y);
    const toSquare = coordsToNotation(to.x, to.y);
    const captureChar = isCapture ? 'x' : '-';
    return `${pieceChar} ${fromSquare}${captureChar}${toSquare}`;
}

function generateDropNotation(pieceType, to) {
    const pieceChar = pieceNotation[pieceType] || pieceType.charAt(0).toUpperCase();
    const toSquare = coordsToNotation(to.x, to.y);
...
            
            // --- ADD NOTATION & LAST MOVE ---
            const turnNum = Math.floor(game.turnCount / 2) + 1;
            const turnPrefix = game.isWhiteTurn ? `${turnNum}. ` : "";
            const notation = generateNotation(piece, from, to, wasCapture);
            game.moveList.push(turnPrefix + notation);
...
    socket.on('makeDrop', (data) => {
		...
		if (game.boardState[to.y][to.x] === null && isPositionValid(to.x, to.y)) {
			...
			if (pieceIndex > -1) {
                ...
                const turnNum = Math.floor(game.turnCount / 2) + 1;
                const turnPrefix = game.isWhiteTurn ? `${turnNum}. ` : "";
                const notation = generateDropNotation(piece.type, to);
                game.moveList.push(turnPrefix + notation);
                game.lastMove = { from: 'hand', to };
                // --- END ADD ---
...
    });
    ...
    socket.on('resignGame', (gameId) => {
        const game = games[gameId];
        if (!game || game.gameOver || game.isSinglePlayer) return;
    ...
    socket.on('offerRematch', (gameId) => {
        const game = games[gameId];
        if (!game || game.gameOver === false || game.isSinglePlayer) return;

        game.rematchOfferedBy = socket.id;
        const opponentSocketId = game.players.white === socket.id ? game.players.black : game.players.white;
        
        io.to(opponentSocketId).emit('rematchOffered');
    });

    socket.on('acceptRematch', (gameId) => {
        const game = games[gameId];
        if (!game || !game.rematchOfferedBy) return;

        const opponentSocketId = game.players.white === socket.id ? game.players.black : game.players.white;

        // Check if the person who accepted is the one who *received* the offer
        if (game.rematchOfferedBy === opponentSocketId) {
            // Reset the game state
            game.boardState = getInitialBoard();
            game.whiteCaptured = [];
            game.blackCaptured = [];
            ...
            game.winner = null;
            game.reason = null;
            game.whiteTimeLeft = game.timeControl.main;
            game.blackTimeLeft = game.timeControl.main;
            game.lastMoveTimestamp = Date.now();
            game.moveList = [];
            game.lastMove = null;
...
                if(!game.gameOver && !game.isSinglePlayer) { // Don't end single player games
                    game.gameOver = true;
                    game.winner = game.players.white === socket.id ? 'black' : 'white';
...
                }
                if (game.isSinglePlayer) {
                    console.log(`Single player game ${gameId} instance removed.`);
                } else {
                    delete lobbyGames[gameId];
                    io.emit('lobbyUpdate', lobbyGames);
