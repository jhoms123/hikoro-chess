...
function updateTimeOnMove(game) {
    ...
    const now = Date.now();
    const timeSpent = (now - game.lastMoveTimestamp) / 1000;
    const movedPlayerColor = !game.isWhiteTurn ? 'white' : 'black';
    let timeLeft = game[`${movedPlayerColor}TimeLeft`];
    timeLeft -= timeSpent;
    if (timeLeft < 0) {
        timeLeft = 0;
    }
    game[`${movedPlayerColor}TimeLeft`] = timeLeft;
...
io.on('connection', (socket) => {
    ...
    socket.on('createGame', (data) => {
        ...
        lobbyGames[gameId] = { id: gameId, creatorName: playerName || 'Anonymous', timeControl: tc };
        socket.join(gameId);
    ...
    socket.on('joinGame', (gameId) => {
        ...
        if (game && !game.players.black) {
            ...
            socket.join(gameId);
            game.lastMoveTimestamp = Date.now(); // Starts the clock
    ...
    socket.on('disconnect', () => {
        ...
        for (const gameId in games) {
            ...
            if (game.players.white === socket.id || game.players.black === socket.id) {
                if(!game.gameOver && !game.isSinglePlayer) {
                    game.gameOver = true;
                    game.winner = game.players.white === socket.id ? 'black' : 'white';
